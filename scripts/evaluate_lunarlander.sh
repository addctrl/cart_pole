#!/usr/bin/env bash
# Wrapper do ewaluacji najlepszego modelu LunarLander.
...
results_path = Path("data/lunarlander_experiments.csv")
...
if not candidates:
	raise SystemExit("Brak ukonczonych eksperymentow w data/lunarlander_experiments.csv")
...
source .venv/bin/activate
python -m src.evaluate --model-path "$model_path" --env-id LunarLander-v3 --episodes "$episodes"
